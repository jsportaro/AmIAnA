    ...
    public class BackPropNeuralNet
{
        ...
        public double[] Train(double[][] initialTrainingData, int maxEpochs,
          double learnRate, double momentum)
        {
            ...
            //Clean
            var badTrainingSet = new List<int>();
            
            for (var i = 0; i < initialTrainingData.Length; i++)
            {
                if (initialTrainingData[i].Any(v => double.IsNaN(v)))
                {
                    if (!badTrainingSet.Contains(i))
                    {
                        badTrainingSet.Add(i);
                    }
                }
            }

            var goodData = new List<double[]>();
            for (var i = 0; i < initialTrainingData.Length; i++)
            {
                if (!badTrainingSet.Contains(i))
                {
                    goodData.Add(initialTrainingData[i]);
                }
            }

            trainData = goodData.ToArray();
            ...
            while (epoch < maxEpochs)
            {
                ...
                {
                    int idx = sequence[ii];
                    Array.Copy(trainData[idx], tValues, numOutput);
                    Array.Copy(trainData[idx], numOutput, xValues, 0, numInput);
                    ...
                    if (ihWeights.SelectMany(ws => ws.Select(w => w)).Any(w => double.IsNaN(w)))
                    {

                    }

                    if (hBiases.Any(w => double.IsNaN(w)))
                    {

                    }

                    if (hoWeights.SelectMany(ws => ws.Select(w => w)).Any(w => double.IsNaN(w)))
                    {

                    }

                    if (oBiases.Any(b => double.IsNaN(b)))
                    {
        ...
        private double Error(double[][] trainData)
        {
            ...
            {
                Array.Copy(trainData[i], tValues, numOutput);
                Array.Copy(trainData[i], numOutput, xValues, 0, numInput);
